...
const RosterComponent = () => {
  ...
  const canModifyDuty = (instructorId: string, date: string, duty?: string): boolean => {
    // Get user identifier - try multiple fields
    const userEmployeeId = currentUser?.employee_id || currentUser?.employeeId || currentUser?.id;
    ...
    if (duty) {
      ...
      if (entry?.created_by && ADMIN_ACCOUNTS.includes(entry.created_by)) {
  ...
  const generateDateColumns = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      columns.push({
        date: day,
        fullDate: `${selectedYear}-${selectedMonth.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`,
        isWeekend,
        dayOfWeek
      });
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.duties && Array.isArray(data.duties)) {
          ...
          const dutyColorsMap = data.duties.reduce((acc: Record<string, string>, d: any) => {
            acc[d.name] = d.color;
            return acc;
          }, {});
  ...
  const fetchInstructors = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const filteredInstructors = users.filter((user: User) => 
        ...
        setError(`Failed to fetch instructors: ${errorData.message || 'Unknown error'}`);
      }
    } catch (err) {
  ...
  const handleScreenshot = async () => {
    try {
      // Use html2canvas library for screenshots
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          ...
          const clonedElement = clonedDoc.querySelector('[data-testid="roster-table"]') || clonedDoc.body;
          if (clonedElement) {
      ...
      const fileName = `教師排程_${selectedYear}年${selectedMonth}月_${new Date().getTime()}.png`;
      ...
      if (error.message?.includes('html2canvas')) {
        alert('截圖庫載入失敗，請確認已安裝 html2canvas 套件');
      } else {
        alert(`截圖失敗：${error.message || '未知錯誤'}，請嘗試手動截圖`);
      }
    }
  };
  ...
  const handleExcelExport = async () => {
    try {
      ...
      instructors.forEach(instructor => {
        const row = [instructor.employee_id, instructor.full_name, instructor.base];
        
        dateColumns.forEach(col => {
          const duties = getDutiesForDate(instructor.employee_id, col.fullDate);
      ...
      const sheetName = `${selectedYear}年${selectedMonth}月排程`;
      XLSX.utils.book_append_sheet(wb, ws, sheetName);

      // Save file
      const fileName = `教師排程_${selectedYear}年${selectedMonth}月_${new Date().getTime()}.xlsx`;
      ...
      if (error.message?.includes('XLSX')) {
        alert('Excel庫載入失敗，請確認已安裝 xlsx 套件');
      } else {
  ...
  const handleDatabaseCleanup = async () => {
    const userEmployeeId = currentUser?.employee_id || currentUser?.employeeId || currentUser?.id;
    if (!ADMIN_ACCOUNTS.includes(userEmployeeId || "")) {
      alert("只有管理員可以執行資料庫清理");
      return;
    }

    if (!confirm("確定要清理資料庫嗎？\n\n這將會：\n• 刪除空的排程記錄\n• 刪除超出年份範圍的記錄\n• 只保留 2025 年至當前年份+1 的記錄")) {
      return;
    }
    ...
    try {
      ...
      if (previewResponse.ok) {
        ...
        const { toBeDeleted, willRemain, currentYearRange } = preview.preview;
        
        const confirmMessage = `清理預覽：\n\n將刪除：\n• ${toBeDeleted.emptyDuties} 個空排程\n• ${toBeDeleted.oldRecords} 個過舊記錄\n• ${toBeDeleted.futureRecords} 個過新記錄\n• 總計：${toBeDeleted.total} 個記錄\n\n將保留：${willRemain} 個記錄 (${currentYearRange})\n\n確定執行清理？`;
  ...
  // Add duty to a date
  const addDutyToDate = async (instructorId: string, date: string, duty: string) => {
    ...
    try {
      ...
      const instructor = instructors.find(i => i.employee_id === instructorId);
      const scheduleEntry = {
        ...
        date,
        duties: newDuties,
        created_by: currentUser?.employee_id || currentUser?.id || currentUser?.id
      };
  ...
  const removeDutyFromDate = async (instructorId: string, date: string, dutyToRemove: string) => {
    ...
    try {
      const currentDuties = getDutiesForDate(instructorId, date);
      const newDuties = currentDuties.filter(duty => duty !== dutyToRemove);
  ...
  // Handle drop (desktop)
  const handleDrop = (e: React.DragEvent, instructorId: string, date: string) => {
    e.preventDefault();
  ...
  const handleDutyClick = (e: React.MouseEvent, instructorId: string, date: string, duty: string) => {
    e.stopPropagation();
  ...
  const addCustomDuty = async (newDuty: string, color: string) => {
    ...
    if (!availableDuties.includes(newDuty)) {
      ...
      console.log('Updated duty colors:', { ...dutyColors, [newDuty]: color });
    }
  };
  ...
  const updateDutyName = (oldName: string, newName: string) => {
    setAvailableDuties(prev => prev.map(d => d === oldName ? newName : d));
    ...
    setScheduleData(prev => {
      ...
      Object.keys(newData).forEach(key => {
        newData[key] = newData[key].map(entry => ({
          ...entry,
          duties: entry.duties.map(duty => duty === oldName ? newName : duty)
        }));
      });
      return newData;
    });
    ...
    <div className={styles.container}>
      ...
      <div className={styles.maxWidth}>
        ...
        <DutyManager
          ...
          onToggleMinimize={() => setDutiesMinimized(!dutiesMinimized)}
          onAddCustomDuty={addCustomDuty}
          onUpdateDuties={updateDuties}
          onUpdateDutyName={updateDutyName}
          isMobile={isMobile}
          isAdmin={ADMIN_ACCOUNTS.includes(currentUser?.employee_id || currentUser?.employeeId || currentUser?.id || "")}
          token={token || ""}
        />
        ...
        {/* Roster Table */}
        <div className={styles.tableContainer} ref={tableContainerRef} data-testid="roster-table">
          <table className={styles.rosterTable}>
            <thead>
              <tr>
                <th className={styles.instructorHeader}>員編</th>
                <th className={styles.instructorHeader}>姓名</th>
                <th className={styles.instructorHeader}>基地</th>
                {dateColumns.map((col) => (
                  <th
                    key={col.date}
                    className={`${styles.dateHeader} ${col.isWeekend ? styles.weekend : ''}`}
                  >
                    <div className={styles.dateHeaderContent}>
                      <div className={styles.dateNumber}>{col.date}</div>
                      <div className={styles.dayOfWeek}>
                        {['日', '一', '二', '三', '四', '五', '六'][col.dayOfWeek]}
                      </div>
                    </div>
                  </th>
                ))}
            ...
            <tbody>
              {instructors.map((instructor) => (
                <tr key={instructor.id} className={styles.instructorRow}>
                  <td className={styles.instructorCell}>{instructor.employee_id}</td>
                  <td className={styles.instructorCell}>{instructor.full_name}</td>
                  <td className={styles.instructorCell}>{instructor.base}</td>
                  {dateColumns.map((col) => {
                    const duties = getDutiesForDate(instructor.employee_id, col.fullDate);
                    const canModify = canModifyDuty(instructor.employee_id, col.fullDate);
                    
                    return (
                      <td 
                        key={col.date} 
                        className={`${styles.scheduleCell} ${!canModify ? styles.readOnlyCell : ''}`}
                        onDrop={(e) => handleDrop(e, instructor.employee_id, col.fullDate)}
                        onDragOver={handleDragOver}
                        onClick={() => handleCellClick(instructor.employee_id, col.fullDate)}
                        title={!canModify ? "您沒有權限修改此排程" : ""}
                      >
                        <div className={styles.dutiesContainer}>
                          {duties.map((duty, index) => (
                            <div
                              key={index}
                              className={`${styles.dutyTag} ${!canModify ? styles.readOnlyDuty : ''}`}
                              style={{ backgroundColor: DUTY_COLORS[duty] || '#3b82f6' }}
                              onClick={(e) => handleDutyClick(e, instructor.employee_id, col.fullDate, duty)}
                              title={
                                !canModify 
                                  ? "管理員設定，無法修改"
                                  : isMobile 
        ...
        <div className={styles.instructions}>
          <h3>使用說明：</h3>
          <ul>
            <li><strong>桌面版本：</strong>拖拉上方任務到對應日期</li>
            <li><strong>手機版本：</strong>點選欲排任務，再點選日期</li>
            <li><strong>刪除任務：</strong>{isMobile ? '點選已排定的任務' : '要刪除的任務用滑鼠點選兩次'}</li>
            <li><strong>權限說明：</strong>一般使用者只能修改自己的班表，管理者才有權限修改所有任務</li>
            <li><strong>鎖定任務：</strong>管理者安排的任務別想刪除!</li>
          </ul>
        </div>
